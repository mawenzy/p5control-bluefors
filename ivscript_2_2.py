...
class MeasurementScript_v2():
    def __init__(self):
        ...
        self.adwin_sample_rate = 4000.0
        self.femto1_amp = 1e0
        self.femto2_amp = 1e0
        self.magnet_rate = 0.0

        self.initial_ramp_cool_down = 600.0
        self.ramp_cool_down = 10.0
        self.heater_cool_down = 10.0
        self.offset_cool_down = 0.0
        self.sweep_cool_down = 0.0
        self.meas_delay_time = 0.5

        self.amplitude = 0.0
        self.period = 28.0327 # equals 35.673mHz, 50Hz equals 20 ms
        self.sweep_time = 30.0
        self.offset_time = 3.0
    ...
    def name_generator(self,**kwargs) -> str:
        class Fields(Enum):
            title = "Untitled"
        ...
        for key_str, value in kwargs.items():
            try:
                ...
                logger.error(f"KeyError: {key_str} is not a valid NameGen key.")
    ...
    def single_IV(
            ...
            gate_voltage = np.nan,
            vna_amplitude = np.nan,
            vna_frequency = np.nan,            
                    ):
        
        # Generate name
        if iv_name is None: iv_name = self.iv_name
    ...
    def gate_study(
        ...
        # Generate Name
        if gate_voltages_name is None: gate_voltages_name = self.gate_voltages_name

        gate_voltages_name = self.name_generator(
            title=gate_voltages_name,
            variable=("gate_voltage", (np.min(gate_voltages),np.max(gate_voltages)),"V"),
            vna_frequency = vna_frequency,
            vna_amplitude = vna_amplitude,
            magnetic_field = magnetic_field
        )
        ...
        try:
            ...
            for _, gate_voltage in enumerate(tqdm(gate_voltages)):
                fname = self.name_generator_old(gate_voltage=gate_voltage)[1:]
    ...
    def magnetic_field_study(
        ...
        # Generate Name
        if magnetic_fields_name is None: magnetic_fields_name = self.magnetic_fields_name

        magnetic_fields_name = self.name_generator(
            title=magnetic_fields_name,
            variable=("magnetic_field", (np.min(magnetic_fields),np.max(magnetic_fields)),"T"),
            gate_voltage=gate_voltage,
            vna_frequency = vna_frequency,
            vna_amplitude = vna_amplitude
        )
        ...
        try:
            ...
            for _, magnetic_field in enumerate(tqdm(magnetic_fields)):
                fname = self.name_generator_old(magnetic_field=magnetic_field)[1:]
    ...
    def irradiation_study(
        ...
        # Generate Name
        if vna_irradiations_name is None: vna_irradiations_name = self.vna_irradiations_name

        vna_irradiations_name = self.name_generator(
            title=vna_irradiations_name,
            variable=("vna_amplitudes_frequencies", (np.nan,np.nan),""),
            gate_voltage=gate_voltage,
            magnetic_field=magnetic_field,
        )    
    ...
    def frequency_study(
        ...
        # Generate Name
        if vna_frequencies_name is None: vna_frequencies_name = self.vna_frequencies_name

        vna_frequencies_name = self.name_generator(
            title=vna_frequencies_name,
            variable=("vna_frequencies", (np.min(vna_frequencies),np.max(vna_frequencies)),"Hz"),
            gate_voltage=gate_voltage,
            vna_amplitude=vna_amplitude,
            magnetic_field=magnetic_field,
        )  
    ...
    def amplitude_study(
        ...
        # Generate Name
        if vna_amplitudes_name is None: vna_amplitudes_name = self.vna_amplitudes_name

        vna_amplitudes_name = self.name_generator(
            title=vna_amplitudes_name,
            variable=("vna_amplitudes", (np.min(vna_amplitudes),np.max(vna_amplitudes)),"V"),
            gate_voltage=gate_voltage,
            vna_frequency=vna_frequency,
            magnetic_field=magnetic_field,
        )
    ...
    def temperature_study(
        ...
        # Generate Name
        if temperatures_name is None: temperatures_name = self.temperatures_name

        temperatures_name = self.name_generator(
            title=temperatures_name,
            variable=("heater_power", (np.min(temperatures),np.max(temperatures)),"T"),
            gate_voltage=gate_voltage,
            vna_frequency=vna_frequency,
            vna_amplitude=vna_amplitude,
            magnetic_field=magnetic_field,
        ...
        try:
            ...
            for _, heater_power in enumerate(tqdm(heater_powers)):
                fname = self.name_generator_old(heater_power=heater_power)[1:]
                if heater_power == 0:
                    fname += f'_{index:03d}'
                    index += 1
