...
Dim sweep_time, sweep_start, sweep_period as Float
Dim sweep_counter as Long
...
''''' start use of waiting time '''''
  ...
  ' sweep
  if (Par_10 = 1) Then
    sweep_period = FPar_13
    sweep_time = time - sweep_start
    sweep_counter = sweep_time/sweep_period
    FPar_25 = (-1 + 2*Mod(sweep_counter, 2))
    FPar_26 = (-1+2/sweep_period*sweep_time)
    sweep_value = FPar_25*FPar_26
  ...
  ' cv
  if (Par_10 = 0) Then
    sweep_time = 0 : sweep_start = time
    sweep_value = 1 : sweep_counter = 0
    trigger = -1 : state = 0
