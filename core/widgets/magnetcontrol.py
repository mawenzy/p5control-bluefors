...
class MagnetControl(QWidget):
    ...
    def __init__(
        ...
        self.state_indicator = DisabledLineEdit()

        self.actual_field_label = QLabel()
        self.actual_field_label.setText("Actual field: (mT)")
        self.actual_field = DisabledLineEdit()

        self.target_field_label = QLabel()
        self.target_field_label.setText("Target field: (mT)")
        ...
        self.rate_label.setText("Target rate: (mT/min)")
        ...
        layout.addWidget(self.actual_field_label, 1, 0, 1, 2)
        layout.addWidget(self.actual_field, 1, 2)

        layout.addWidget(self.target_field_label, 2, 0, 1, 2)
        layout.addWidget(self.target_field, 2, 2)

        layout.addWidget(self.rate_label, 3, 0, 1, 2)
        layout.addWidget(self.rate, 3, 2)
    ...
    def _handle_status_callback(self, arr):
        ...
        self.state_indicator.setText(f"{state}")
